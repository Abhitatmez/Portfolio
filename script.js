...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add this new code for the Help menu
    const helpMenu = document.querySelector('.menu-item');
    ...
    // Add this function to handle the About window controls
    const aboutWindow = document.getElementById('about-window');
    
    // Function to open window with animation
    function openAboutWindow() {
        aboutWindow.style.display = 'block';
        requestAnimationFrame(() => {
            aboutWindow.classList.add('opening');
        });
    }

    // Add click handler to About Me icon
    const aboutIcon = document.querySelector('.app-icon[data-app="about"]');
    aboutIcon.addEventListener('dblclick', (e) => {
    ...
    closeBtn.addEventListener('click', () => {
        aboutWindow.classList.remove('opening');
        aboutWindow.classList.add('closing');
        
        aboutWindow.addEventListener('animationend', function handleClose() {
            aboutWindow.style.display = 'none';
            aboutWindow.classList.remove('closing');
            aboutWindow.removeEventListener('animationend', handleClose);
        });
    });

    // Yellow circle (Minimize)
    const minimizeBtn = aboutWindow.querySelector('.minimize');
    minimizeBtn.addEventListener('click', () => {
        aboutWindow.classList.remove('opening');
        aboutWindow.classList.add('minimizing');
        
        aboutWindow.addEventListener('animationend', function handleMinimize() {
            aboutWindow.style.display = 'none';
            aboutWindow.classList.remove('minimizing');
            aboutWindow.removeEventListener('animationend', handleMinimize);
        });
    });

    // Green circle (Maximize)
    const maximizeBtn = aboutWindow.querySelector('.maximize');
    let messageVisible = false; // Track message visibility

    maximizeBtn.addEventListener('click', () => {
        let message = aboutWindow.querySelector('.fullscreen-message');
        ...
        if (!message) {
            ...
            message.textContent = 'This window cannot be maximized';
            message.style.opacity = '0';
            message.style.transition = 'opacity 0.3s';
            aboutWindow.appendChild(message);
        }
    ...
    const apps = ['about', 'projects', 'certificates', 'skills', 'resume'];
    ...
    apps.forEach(appName => {
        ...
        if (icon && window) {
            // Double click handler for icon
            icon.addEventListener('dblclick', (e) => {
                console.log(`Double clicked ${appName}`); // Debug log
                if (!document.getElementById('drag-toggle').checked) {
                    openWindow(window);
                }
            });
            ...
            console.warn(`Missing elements for ${appName}:`, {
                window: !!window,
                icon: !!icon
            });
        }
    ...
    function updateSection(direction) {
        ...
        nextBtn.textContent = currentSection === totalSections ? 'Finish' : 'Next';
        pageIndicator.textContent = `${currentSection}/${totalSections}`;
    }
    
    prevBtn.addEventListener('click', () => {
        if (currentSection > 1) updateSection(-1);
    });
...
document.getElementById('theme-toggle').addEventListener('change', function(e) {
    if (e.target.checked) {
        console.log('Switching to dark mode, using wifiwhite.svg');
        document.querySelector('.wifi-icon img').src = './assets/wifiwhite.svg';
    } else {
        console.log('Switching to light mode, using wifi.svg');
        document.querySelector('.wifi-icon img').src = './assets/wifi.svg';
    }
});
...
function setupWindowControls(window) {
    ...
    closeBtn.addEventListener('click', () => {
        ...
        window.addEventListener('animationend', function handleClose() {
            window.style.display = 'none';
            window.classList.remove('closing');
            window.removeEventListener('animationend', handleClose);
        });
    });
    ...
    minimizeBtn.addEventListener('click', () => {
        ...
        window.addEventListener('animationend', function handleMinimize() {
            window.style.display = 'none';
            window.classList.remove('minimizing');
            window.removeEventListener('animationend', handleMinimize);
        });
    });
    ...
    maximizeBtn.addEventListener('click', () => {
        let message = window.querySelector('.fullscreen-message');
